...
export default async function CustomersPage({ searchParams }: PageProps<"/customers">) {
  ...
  const ohneZugriff = customers.filter((c) => c.issues.length).length;
  ...
  return (
    <>
      <Blattkopf
        ...
        meaning="customers"
        figur={rows.length}
        figurEinheit={rows.length === 1 ? "Kunde" : "Kunden"}
        stand={
          q
            ? `von ${customers.length} im Portfolio · gefiltert nach „${q}"`
            : ohneZugriff > 0
              ? `${ohneZugriff} ${ohneZugriff === 1 ? "Kunde wartet" : "Kunden warten"} auf eine Freigabe.`
              : "Alle Seiten und Werbekonten sind freigegeben."
        }
      />
